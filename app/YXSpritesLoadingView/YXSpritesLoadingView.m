...
+ (void)showandVC:(UIViewController *)vc
{
	[[self sharedInstance] loadingViewSetupWithText:nil andShimmering:YES andBlur:YES andVC:vc];
}

+ (void)showWithText:(NSString *)text andVC:(UIViewController *)vc
{
    [[self sharedInstance] loadingViewSetupWithText:text andShimmering:YES andBlur:YES andVC:vc];

}

+ (void)showWithText:(NSString *)text andShimmering:(BOOL)shimmering andBlurEffect:(BOOL)blur andVC:(UIViewController *)vc
{
    [[self sharedInstance] loadingViewSetupWithText:text andShimmering:shimmering andBlur:blur andVC:vc];
}
...
- (void)loadingViewSetupWithText:(NSString *)text andShimmering:(BOOL)shimmering andBlur:(BOOL)blur andVC:(UIViewController *)vc
{
    if (!loadingImageView)
    {
        ...
        if (text && ![text isEqualToString:@""]) {
            ...
            loaderView.frame = CGRectMake((self.frame.size.width - loaderBackgroundWidth) / 2,
                                              (self.frame.size.height - loaderBackgroundHeight - height) / 2 - adjustHeightForLoader-64,
        ...
        {
            loaderView.frame = CGRectMake((self.frame.size.width - loaderBackgroundWidth) / 2, (self.frame.size.height - loaderBackgroundWidth) / 2 - adjustHeightForLoader-64, loaderBackgroundWidth, loaderBackgroundWidth);
        }
        
        loaderView.layer.cornerRadius = loaderCornerRadius;
        loaderView.layer.shadowOffset=CGSizeMake(2, 2);
        loaderView.layer.shadowColor=[UIColor blackColor].CGColor;
        loaderView.layer.shadowOpacity=0.6;
        loaderView.layer.shadowRadius=2;
